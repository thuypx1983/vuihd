...
//	$info_name_en = explode('</span>', $info_name_en[1]);
	...
	$quocgia = $countryid;
}elseif ($_POST['webgrab'] == 'yuphim') {
	$info_url_html = xem_web($_POST['urlgrab']);
    $info_link = explode('<a href="http://yuphim.net/xem-phim/', $info_url_html);
	$info_link = explode('"', $info_link[1]);
	
	$url_play_phims = "http://yuphim.net/xem-phim/".$info_link[0];
	$url_play_phim = xem_web($url_play_phims);
	...
	$info_img = explode('<img class="thumb" src="', $info_url_html);
	$info_img = explode('"', $info_img[1]);
	$info_img = 'http://yuphim.net/'.$info_img[0];
...
}elseif ($_POST['webgrab'] == 'phimplus') {
	$info_url_html = xem_web($_POST['urlgrab']);
    $info_link = explode('<a href="http://phimplus.net/xem-phim/', $info_url_html);
	$info_link = explode('"', $info_link[1]);
	
	$url_play_phims = "http://phimplus.net/xem-phim/".$info_link[0];
	$url_play_phim = xem_web($url_play_phims);
	...
	$info_img = explode('<img class="thumb" src="', $info_url_html);
	$info_img = explode('"', $info_img[1]);
	$info_img = 'http://phimplus.net/'.$info_img[0];
...
<div class="form-group">
                    ...
                    for ($i=$episode_begin;$i<=$episode_end;$i++) {
						if ($_POST['webgrab'] == 'phimplus') {
							$play_url = explode('"', $url_play[$i]);
							$play_url = 'http://phimplus.net/xem-phim/'.$play_url[0];
							...
							$play_url = explode('"', $url_play[$i]);
							$play_url = 'http://yuphim.net/xml/'.$play_url[0].'.xml';
							$html_link_play = xem_web($play_url);
							$link_phim = explode("<location>", $html_link_play);
							$link_phim = explode("</location>", $link_phim[1]);
							...
							$name = explode('</title>', $name[1]);
							if($name[0] == "Fuil HD") $name[0] = "Full";
						}elseif ($_POST['webgrab'] == 'xemxinetv') {
							$play_url = explode('"', $url_play[$i]);
							$play_url = 'http://xemxine.tv/xml/'.$play_url[0].'.xml';
							$html_link_play = xem_web($play_url);
							$link_phim = explode("<location>", $html_link_play);
							$link_phim = explode("</location>", $link_phim[1]);
