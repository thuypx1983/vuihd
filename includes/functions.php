...
function plsendmail($content,$contentHtml){
    ...
    $mail->Subject = "Request box from website";
...
function playTech_check($url){
    if(strpos($url , 'kisscartoon.me') !== false){
        ...
        $playTech = 'flashv1';
    }elseif(strpos($url , 'sv3.vnhd.net') !== false){
        $playTech = 'flash';
...
function checkExistShared(){

}
function addShared(){
    $sql="INSERT INTO `table_user_vietlott`
             (`uv_id`, `uv_user_id`, `uv_facebook_url`, `uv_time`) 
             VALUES 
             (NULL, {$_SESSION['user_id']}, '1', ".time().");";
    $mysql = new mysql;
}
...
function getNextDate(){
    ...
    $now=time();
    if($now>$date){
        $current=strtotime(date("Y-m-d 20:00:00", strtotime( '+1 days' ) ));
    }else{
        $current=strtotime(date("Y-m-d 20:00:00"));
    }
    return $current;
}
...
function getDates(){
    ...
    $now=time();
    if($now>$date){
        $startDate=strtotime(date("Y-m-d 20:00:00" ));
        $endDate=strtotime(date("Y-m-d 20:00:00", strtotime( '+1 days' ) ));
    }else{
        $startDate=strtotime(date("Y-m-d 20:00:00",strtotime( '-1 days' ) ));
        $endDate=strtotime(date("Y-m-d 20:00:00"));
    }
    return array($startDate,$endDate);
}

function checkShared($user_id){
    global $mysql;
    list($startTime,$endTime)=getDates();
    $sql="SELECT * FROM ".DATABASE_FX."user_vietlott
     WHERE `uv_user_id`={$user_id} AND (`uv_time`>{$startTime} AND `uv_time`<{$endTime}) LIMIT 1";
    $row= $mysql->query($sql);
    if($row->rowCount()==0){
        return false;
    }else{
...
function insertShared($user_id){
    global $mysql;
    $sql="INSERT INTO ".DATABASE_FX."user_vietlott(`uv_user_id`, `uv_time`)
    VALUES ('{$user_id}','".time()."');
    ";
    $mysql->query($sql);
}
function insertNumbers($uv_id,$number1,$number2,$number3){
    global $mysql;
    $sql="UPDATE ".DATABASE_FX."user_vietlott SET 
    `number1`=".(int)$number1.", 
    `number2`=".(int)$number2.",
    `number3`=".(int)$number3." 
     WHERE `uv_id`={$uv_id}
