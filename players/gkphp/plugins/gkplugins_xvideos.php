...
class gkplugins_xvideos extends gkpluginsModel
{
    ...
    {
        $rs = false;
        if (strpos($link, "http://www.xvideos.com/") !== false || strpos($link, "http://xvideos.com/") !== false || strpos($link, "http://jp.xvideos.com/") !== false) {
            $rs = true;
        }
        return $rs;
    }
    ...
    private function getMp4($link)
    {
        $dta = $this->get_curl(array(
            "url" => $link,
            "showHeader" => true,
            "agent" => "Mozilla/5.0 (iPhone; CPU iPhone OS 7_1_1 like Mac OS X) AppleWebKit/537.51.2 (KHTML, like Gecko) CriOS/39.0.2171.50 Mobile/11D201 Safari/9537.53"
        ));
        $this->getMp4Com($dta);
    }
