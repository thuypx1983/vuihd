...
{
    //==========
    private $defaultQuality = "720p"; //360p,480p,720p,1080p
    private $cache_dir = '../../cache/cache2/';
    private $time_cache = 1800;//s
    //==========
    ...
    private function getTV($dta, $url)
    {
        if (file_exists($this->cache_dir . md5($url)) && (filemtime($this->cache_dir . md5($url))>=time()-$this->time_cache))
		{
			$linkarr =(array) json_decode( file_get_contents($this->cache_dir . md5($url)) );
		} else
		{
