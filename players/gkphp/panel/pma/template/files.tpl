...
echo "<div class='left'>".$_var->home.($_GET['db'] ? "<a href='tables.php?db=".urlencode($db_name)."'>".pma_img('database.png').htmlentities($db_name)."</a> &#187;" : "")."<a href='?'>".pma_img("files.png")." $lang->Files </a> &#187; ".pma_img("b_drop.png")."$lang->delete</div><hr size='1px'>";
	...
	if($_POST['ok'])
		...
		printf("<form action='?act=delete&db=".$_GET['db']."' method='post'>".$lang->You_selected_files."<br/> <i>".implode("<br/>",$_POST['i'])."</i> $_m <br/><br/>".$lang->DELETE_SELECTED."? <br/><br/><input type='submit' name='ok' value='$lang->Yes'><a href='?'>$lang->No</a></form>",count($_POST['i']));
...
if($_total > 0){
	foreach($fl_dt as $fl){
	echo pma_img("file.png")."<input type='checkbox' name='i[]' value='$fl'> <a href='$fl'>".basename($fl)."</a> (".convert(filesize($fl)).") ".pma_img("b_tblimport.png")." <a href='import.php?".($db_name ? "db=".urlencode($db_name) : "")."&file=$fl&th=1'>$lang->Import </a><br/>";
	}
echo "<input type='checkbox' onClick='toggle(this)'>".$lang->With_selected.":<br/> <input type='submit' value='".$lang->delete."'>";
echo '<br/>
<font color=red><b>Gợi ý</b></font><br/>
+ Hãy dùng ftp để zip CSDL này lại rồi tải về máy
<br/>';
